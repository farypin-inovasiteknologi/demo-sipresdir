...
function initTenant() {
    ...
    if (isElectron) {
        // Jika berjalan di Desktop (Offline), API_URL tidak diperlukan karena kita menggunakan IPC.
...
async function fetchAPI(action, params = {}) {
    ...
    if (currentUser && currentUser.token && !params.token) params.token = currentUser.token;
    try {
        // Cek jika berjalan di aplikasi Desktop (Offline Mode) dengan IPC
        if (window.electronAPI) {
            return await window.electronAPI.queryDB(action, params);
        }

        // Berjalan di Web (Online Mode) via Google Apps Script
...
async function initAppConfigs() {
    try {
        ...
        const settingsResponse = window.electronAPI ? await fetchAPI('getAppConfig') : await fetchAPI('getLinkSettings');
...
async function requirePasswordChange(user) {
    ...
    localStorage.setItem('absensiAppSession', JSON.stringify(user));
