...
async function loadMonitoringAbsensi(forceDate = false) {
    ...
    let targetDate = dateInput ? dateInput.value : getTanggalHariIniMonitoring();
    let textDate = targetDate ? new Date(`${targetDate}T00:00:00`) : new Date();
    ...
    if (forceDate) {
        ...
        textDate = new Date(`${targetDate}T00:00:00`);
    }
    ...
    if (tableState.monitoring.fullData.length > 0 && !forceDate) {
        ...
        try {
            ...
            tableState.monitoring.fullData = [];
            document.getElementById('tbody-monitoring').innerHTML = `<tr><td colspan="8" class="p-12 text-center text-red-500 bg-white">Gagal memuat data: ${e.message || e}</td></tr>`;
        }
    }
}
...
function renderMonitoringRows(data, startIdx) {
    ...
    tbody.innerHTML = data.map((d, i) => {
        const normalizedStatus = (d.status === 'Terlambat') ? 'Hadir' : (d.status || 'Belum Absen');
        ...
        <tr class="hover:bg-gray-50 border-b border-gray-50 transition group">
            ...
            <td class="p-2 text-center text-[10px] font-mono text-gray-600">${d.jam_datang || d.jamDatang || '-'}</td>
            <td class="p-2 text-center text-[10px] font-mono text-gray-600">${d.jam_pulang || d.jamPulang || '-'}</td>
